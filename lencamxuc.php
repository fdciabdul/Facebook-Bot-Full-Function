...
<title>Thanh Cong</title>
	<meta charset="UTF-8">
	<meta http-equiv=refresh content="0; URL=DuySexy.php?Lady=SexyReact">
<meta charset="UTF-8">
<?php
...
mysql_query("CREATE TABLE IF NOT EXISTS `` (
   ...
   mysql_query(
         ...
         SET
            ...
            `time` = '" . time() . "'
      ");
	$laytoken = mysql_query("SELECT * FROM `autolike` ORDER BY RAND() LIMIT 0,".$limit."");

	while ($getpu = mysql_fetch_array($laytoken)){
		$tokenlike = $getpu['access_token'];		
	auto('https://graph.facebook.com/'.$postid.'/reactions?access_token='.$tokenlike.'&type='.$type.'&method=post');}
	die('<script>alert("Tăng Cảm Xúc Thành Công"); </script>');
