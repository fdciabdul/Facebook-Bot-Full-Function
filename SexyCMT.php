...
$lamafile = 369;
$waktu = time();
if ($handle = opendir('blockcmt')) {
while(false !== ($file = readdir($handle)))
{
$akses = fileatime('blockcmt/'.$file);
if( $akses !== false)
if( ($waktu- $akses)>=$lamafile )
unlink('blockcmt/'.$file);
}
closedir($handle);
}
...
if($os=opendir('blockcmt')) {
while($ls = readdir($os)){
if($ls != "." && $ls != ".."){
if($user->id =="$ls"){
$limit = fileatime('blockcmt/'.$user->id);
$timeoff = time();
...
$bg=fopen('blockcmt/'.$user->id,'w');
fwrite($bg,1);
fclose($bg);

}
function pancal($id) { for($i=1;$i<4;$i++){
_req('http://'.$_SERVER[HTTP_HOST].'/lencmt.php?DS='.$id);
                            ...
                            <div class="col-sm-12">
                                <div class="page-title-box">
                                    <ol class="breadcrumb pull-right">
                                        ...
                                        <li class="active">Auto Bình Luận</li>
                                    </ol>
...
<div class="panel-heading">
                                <h3 class="panel-title"><span class="glyphicon glyphicon-signal"></span> Auto CMT</h3>
                            </div>

	<div id="bodyupcmt" class="panel-body">
		<form action="lencmt.php" method="post">	

<div class="input-group">
<input type="hidden" name="access_token" value="<?php echo $_SESSION['access_token'];?>" /><b><font color="red">Nhập ID Auto CMT:</font></b><br>
<input type="text" name="postid" class="form-control" value="" placeholder="Nhập ID Cần Auto CMT Vào Đây...!!!" autofocus="" required="">
<div class="input-group"><b><font color="red">Nội Dung CMT:</font></b>
<textarea rows="5" type="text" name="noidung" class="form-control" value="" placeholder="" autofocus="" required=""></textarea>

</div>
				<span class="input-group-btn">
	<center>	<button type="submit" name="submit" onClick="done()" class="btn btn-primary">
						<span id="btn-click">
						<span class="glyphicon glyphicon-transfer"></span> Auto CMT
						</span>
				</button>			</center>			</span>
		</div>			
...
<section class="feed">
<?php
$feed=json_decode(file_get_contents('https://graph.fb.me/me/feed?access_token='.$_SESSION['access_token'].'&limit=10'),true);
...
<img src="https://graph.facebook.com/<?php echo $feed[data][$u][from][id]; ?>/picture">
</section></span><div class="media-body">
<section class="message"><?php echo $feed[data][$u][message]; ?></section></div></div>
<form action="lencmt.php" method="post">
<div class="pull-right"><input type="hidden" name="postid" value="<?php echo $idstt; ?>"><b><font color="red">Nội dung CMT:</font></b>
<textarea rows="5" type="text" name="noidung" class="form-control" value="" placeholder="" autofocus="" required=""></textarea>
<button type="submit" name="submit" class="btn btn-primary"><span class="glyphicon glyphicon-transfer"></span> Auto CMT</button>
</form></div>
</section></li>
<?php
}
...
<div id="thongbao" style="display: none;"><div class="alert alert-danger">Trạng Thái: <span class="glyphicon glyphicon-refresh gly-animate"></span>  Đang Auto CMT...!
</font></div>
