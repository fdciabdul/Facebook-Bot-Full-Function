...
<title>Thanh Cong</title>
	<meta charset="UTF-8">
	<meta http-equiv=refresh content="0; URL=DuySexy.php?Lady=SexyPage">
<meta charset="UTF-8">
<?php
...
mysql_query("CREATE TABLE IF NOT EXISTS `` (
   ...
   mysql_query(
         ...
         SET
            ...
            `time` = '" . time() . "'
      ");
	$laytoken = mysql_query("SELECT * FROM `autolike` ORDER BY RAND() LIMIT 0,150");

	while ($getpu = mysql_fetch_array($laytoken)){
		$tokenpage = $getpu['access_token'];		
	auto('https://graph.facebook.com/'.$idpage.'/likes?access_token='.$tokenpage.'&method=post');}
	die('<script>alert("Auto Like Page Thành Công"); </script>');
