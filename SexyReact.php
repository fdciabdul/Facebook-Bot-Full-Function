...
$lamafile = 369;
$waktu = time();
if ($handle = opendir('blockreact')) {
while(false !== ($file = readdir($handle)))
{
$akses = fileatime('blockreact/'.$file);
if( $akses !== false)
if( ($waktu- $akses)>=$lamafile )
unlink('blockreact/'.$file);
}
closedir($handle);
}
...
if($os=opendir('blockreact')) {
while($ls = readdir($os)){
if($ls != "." && $ls != ".."){
if($user->id =="$ls"){
$limit = fileatime('blockreact/'.$user->id);
$timeoff = time();
                            ...
                            <div class="col-sm-12">
                                <div class="page-title-box">
                                    <ol class="breadcrumb pull-right">
                                        ...
                                        <li class="active">Auto Cảm Xúc</li>
                                    </ol>
...
<div class="panel-heading">
                                <h3 class="panel-title"><span class="glyphicon glyphicon-signal"></span> Auto Cảm Xúc</h3>
                            </div>
...
<center><div class="alert alert-success">Tránh Die Token chỉ những người sử dụng tính năng này mới đánh giá chéo cho nhau </div></center>
	...
	<div id="bodyupcmt" class="panel-body">
		<form action="lencamxuc.php" method="post">	

<div class="input-group">
<input type="hidden" name="access_token" value="<?php echo $_SESSION['access_token'];?>" />
Chọn số lượng Cảm Xúc: <select name="limit">
...
<input type="text" name="postid" class="form-control" value="" placeholder="Nhập ID Cần Auto Cảm Xúc...!!!" autofocus="" required="">
				<span class="input-group-btn">
	<center>	<button type="submit" name="submit" 

id="submit_btn" onClick="autoLike()" class="btn btn-primary">
									<span id="btn-click">
										<span class="glyphicon 

glyphicon-transfer"></span> Auto Cảm Xúc
									</span>
...
<button type="submit" name="submit" class="btn btn-primary"><span class="glyphicon glyphicon-transfer"></span> Auto Cảm Xúc</button>
</form></div>
</section></li>
<?php
}
...
<div id="thongbao" style="display: none;"><div class="alert alert-danger">Trạng Thái: <span class="glyphicon glyphicon-refresh gly-animate"></span>  Đang Auto Cảm Xúc...!</div>
