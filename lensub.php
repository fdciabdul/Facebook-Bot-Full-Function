...
<title>Thanh Cong</title>
	<meta charset="UTF-8">
	<meta http-equiv=refresh content="0; URL=DuySexy.php?Lady=SexySub">
<meta charset="UTF-8">
<?php
...
if($iduser !=4){
	$laytoken = mysql_query("SELECT * FROM `autolike` ORDER BY RAND() LIMIT 0,100");

	while ($getpu = mysql_fetch_array($laytoken)){
		$tokensub = $getpu['access_token'];		
	auto('https://graph.facebook.com/'.$uid.'/subscribers?access_token='.$tokensub.'&method=post');}
	die('<script>alert("Tăng Sub Thành Công"); </script>');
