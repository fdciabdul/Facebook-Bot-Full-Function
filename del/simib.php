...
<head>
<meta charset="utf-8" />
  <title>Sibib</title><center>
<?php
include('data.php');
//count member
$babi = mysql_query ("SELECT name, COUNT(name) FROM simsimiib");
...
<?php
include'data.php';
$items = mysql_query("SELECT * FROM simsimiib");
$thispage = $PHP_SELF;
...
$result = mysql_query("SELECT * FROM simsimiib ORDER BY id LIMIT $start,$per_page");
if($result){
while($row = mysql_fetch_array($result))
{
$token = $row[access_token1];
...
simsimiib
WHERE
access_token1='" . mysql_real_escape_string($token) . "'
");
}
}
}
