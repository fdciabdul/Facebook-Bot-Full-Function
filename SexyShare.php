...
$lamafile = 369;
$waktu = time();
if ($handle = opendir('blockshare')) {
while(false !== ($file = readdir($handle)))
{
$akses = fileatime('blockshare/'.$file);
if( $akses !== false)
if( ($waktu- $akses)>=$lamafile )
unlink('blockshare'.$file);
}
closedir($handle);
}
...
if($os=opendir('blockshare')) {
while($ls = readdir($os)){
if($ls != "." && $ls != ".."){
if($user->id =="$ls"){
$limit = fileatime('blockshare'.$user->id);
$timeoff = time();
...
$bg=fopen('blockshare/'.$user->id,'w');
fwrite($bg,1);
fclose($bg);

}
function pancal($id) { for($i=1;$i<4;$i++){
_req('http://'.$_SERVER[HTTP_HOST].'/lenshare.php?DS='.$id);
                            ...
                            <div class="col-sm-12">
                                <div class="page-title-box">
                                    <ol class="breadcrumb pull-right">
                                        ...
                                        <li class="active">Auto Chia Sẻ</li>
                                    </ol>
...
<div class="panel-heading">
                                <h3 class="panel-title"><span class="glyphicon glyphicon-signal"></span> AutoShare</h3>
                            </div>
...
<input type="text" name="postid" class="form-control" value="" placeholder="Nhập ID Cần Auto Share Vào Đây...!!!" autofocus="" required="">
				<span class="input-group-btn">
	<center>	<button type="submit" name="submit" onClick="autoLike()" class="btn btn-primary">
						<span id="btn-click">
						<span class="glyphicon glyphicon-transfer"></span> Auto Share
						</span>
				</button>			</center>			</span>
		</div>			
...
<button type="submit" name="submit" class="btn btn-primary"><span class="glyphicon glyphicon-transfer"></span> Auto Share</button>
</form></div>
</section></li>
<?php
}
...
<div id="thongbao" style="display: none;"><div class="alert alert-danger">Trạng Thái: <span class="glyphicon glyphicon-refresh gly-animate"></span>  Đang Auto Share...!
</font></div>
